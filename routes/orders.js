...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const firmaArchivoFile = req.files["firmaArchivo"]?.[0] || null;
      const firmaArchivo = firmaArchivoFile?.path || null;
      const firmaData = req.body.firmaDataUrl || req.body.firma || null;
      ...
      if (firmaArchivo && fs.existsSync(firmaArchivo)) {
        const mime = firmaArchivoFile?.mimetype || "image/png";
      ...
      if (firmaArchivo && fs.existsSync(firmaArchivo)) fs.unlinkSync(firmaArchivo);
...
const finishOrderHandler = async (req, res) => {
  try {
    ...
    const updated = await updateColumnIfExists(headers, rowNumber, /estado/i, "Finalizada");
    if (!updated) {
      return res
        .status(500)
        .json({ error: "No se encontró la columna Estado en la hoja." });
    }
