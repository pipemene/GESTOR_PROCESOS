...
router.post("/:codigo/finish", async (req, res) => {
  try {
    const { codigo } = req.params;
    const found = await findRowByCode(codigo);
    if (!found) return res.status(404).json({ error: "Orden no encontrada" import express from "express";
import multer from "multer";
import { getSheetData, updateCell, appendRow } from "../services/sheetsService.js";
import {
  uploadBase64ImageToDrive,
...
router.post("/", async (req, res) => {
  try {
    ...
    // 🧩 Orden de columnas según tu hoja de cálculo:
    // A: Cliente | B: Fecha | C: Inquilino | D: Teléfono | E: Código
    // F: Descripción | G: Técnico | H: Estado | I–L: (Fotos, firma, firmante)
    const nuevaFila = [
      arrendatario || "SIN_CLIENTE", // Cliente
      new Date().toLocaleString("es-CO"), // Fecha
      arrendatario || "SIN_INQUILINO", // Inquilino
      telefono || "SIN_TEL", // Teléfono
      codigo || "SIN_CODIGO", // Código
      descripcionFinal || "SIN_DESC", // Descripción
      tecnico || "Sin asignar", // Técnico
      estado, // Estado
      "", "", "", "" // FotoAntes, FotoDespués, Firma, NombreFirmante
    ];
